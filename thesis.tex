...
To tackle this challenge, we have begun exploring the efficacy of a POMDP approach. This expands upon the Bayes' Filter approach by adding responsive actions the robotic agent can take to affect the world and user's state. In our original model, when interacting with the robot, we simply hard coded thresholds at which to perform certain actions, such as pointing, picking, smiling, etc. By adding the POMDP layer, we could train the robot to provide the user with clarifying gestures when it is confused about he user state, and act appropriately otherwise. This would allow for the encoding of a wider range of actions that could be executed when appropriate based on training.
